...
int portNumber = 3333;
char btcString[80] = "3BsM8yWbj9fmeFgbgmTckZBR1Uv3P2YEN3";
int GMTzone = 8; // GMT+8
...
void init_WifiManager()
{
    ...
    // Run if we need a configuration
  {
    ...
    if (!wm.startConfigPortal("MiniBitcoinCollider","88888888"))
    {
  ...
  else
  {
    ...
    wm.setCaptivePortalEnable(false); // disable captive portal redirection
    if (!wm.autoConnect("MiniBitcoinCollider","88888888"))
    {
      Serial.println("Failed to connect and hit timeout");
      //delay(3000);
